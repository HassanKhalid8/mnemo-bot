...
Layered on top of that core:
  ...
  * **Persistence** (`db.py`) — every turn appended to the in-memory list is
    mirrored into SQLite, so history survives closing the tab *and* restarting
    the server.
...
def build_system_prompt(conversation=None, use_memories=True):
    """The persona, plus anything we've learned about the user, plus a summary
    of turns that have been trimmed out of the transcript."""
    parts = [(conversation or {}).get("system_prompt") or SYSTEM_PROMPT]

    if use_memories:
        facts = [m["text"] for m in db.list_memories()]
        if facts:
            parts.append(
...
def new_conversation():
    ...
    conversation_histories[conversation_id] = []
    return jsonify({"conversation": db.get_conversation(conversation_id)}), 201
...
def end_incognito(incognito_id):
...
def chat():
    ...
    if not conversation_id or not db.get_conversation(conversation_id):
        if not user_message:
            return jsonify({"error": "Message cannot be empty."}), 400
        conversation_id = db.create_conversation(derive_title(user_message))
        conversation_histories[conversation_id] = []
        created_new = True
    else:
    ...
    if rewind_to:
        ...
        db.update_conversation(conversation_id, summary=None, summarized_through=0)
    ...
    if regenerate_from:
        ...
        if not created_new and not history and conversation["title"] == "New chat":
            db.update_conversation(conversation_id, title=derive_title(user_message))
...
def stream_reply(*, history, conversation, conversation_id, incognito_id, model,
                 user_row, remember, use_memories, created_new=False):
    """Shared SSE generator for a normal turn, a regenerate, and incognito."""
    skip = (conversation or {}).get("summarized_through") or 0
    contents = build_contents(history, skip)
    system_instruction = build_system_prompt(conversation, use_memories)

    def generate():
        yield sse("start", {
            "conversation_id": conversation_id,
            "incognito": bool(incognito_id),
        ...
        if conversation_id:
            payload["title"] = db.get_conversation(conversation_id)["title"]
            # Background work: learn durable facts, and trim the transcript if
            # it's getting long. Both are best-effort and never block the reply.
            if remember:
                threading.Thread(
                    target=learn_from_turn,
                    args=(conversation_id, history[-2:] if len(history) >= 2 else history),
                    daemon=True,
                ).start()
            threading.Thread(
                target=maybe_summarise, args=(conversation_id, model), daemon=True
            ).start()
...
def reset():
    ...
    conversation_id it wipes every conversation."""
    data = request.get_json(force=True, silent=True) or {}
    conversation_id = data.get("conversation_id")
...
def context():
    ...
    model = conversation.get("model") or MODEL
    return jsonify({
        "context": measure_context(history, build_system_prompt(conversation), model)
    })


def maybe_summarise(conversation_id, model):
    ...
    try:
        ...
        measured = measure_context(history, build_system_prompt(conversation), model)
        ...
        if summary:
            db.update_conversation(
                conversation_id, summary=summary, summarized_through=cutoff
            )
    except Exception as e:
...
def learn_from_turn(conversation_id, turns):
    ...
    try:
        ...
        for fact in facts[:5]:
            ...
            if _is_near_duplicate(fact, fingerprints):
                continue
            db.add_memory(fact, conversation_id)
...
def _jaccard_dedupe():
    ...
    memories = sorted(db.list_memories(), key=lambda m: -len(m["text"]))
    kept, removed = [], 0
...
def dedupe_memories():
    ...
    try:
        ...
        db.clear_memories()
        for fact in merged:
            db.add_memory(fact[:300])
        removed = len(before) - len(db.list_memories())
    except Exception as e:
        print(f"[memory] consolidation fell back to word overlap: {e}")
        removed = _jaccard_dedupe()

    return jsonify({"removed": removed, "memories": db.list_memories()})


@app.route("/api/memories", methods=["GET"])
def get_memories():
    return jsonify({"memories": db.list_memories()})
...
def create_memory():
    ...
    db.add_memory(text[:300])
    return jsonify({"memories": db.list_memories()}), 201
...
def search():
    ...
    return jsonify({"query": query, "results": db.search_messages(query)})
